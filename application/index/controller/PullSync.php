...
class PullSync extends Controller{
    // 同步所有数据
    ...
    public function pull(){
        ...
        if($loginData['success']){
            ...
            echo "<script>confirm('调用下拉登录接口失败')</script>";
        }
    }
    // 下拉任务接口
    ...
    private function pullFinishSuccessJson($tokenId,$pullTaskData){
        // 判断guid是否存在
        if(isset($pullTaskData['guid'])){
            $guid = $pullTaskData['guid'];
        }else{
            // 是机构信息
            if($pullTaskData["objectType"] == "TARGET_ORGANIZATION"){
                $guid = $pullTaskData['data']['code'];
            }else{
                // 是用户信息
                $guid = $pullTaskData['data']['_user'];
            }
        }
        $pullFinishSuccess = [
            "tokenId" => $tokenId,
